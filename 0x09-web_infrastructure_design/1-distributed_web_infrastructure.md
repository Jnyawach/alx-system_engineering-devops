...
This is a distributed web infrastructure that atttempts to balance the incoming traffic to the primary server by distributing some of the load to a replica server with the aid of a server responsible for balancing the load between the two servers (primary and replica).
...
* The server pauses and security risk since no SSL certificate has been installed. This means that
data is tranferred over insecure network which is vaulerable to attack by hackers
* There is no way of knowing the status of each server since no monitoring tool has been configured
